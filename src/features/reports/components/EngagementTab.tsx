...
export const EngagementTab = () => (
  <div className="space-y-8">
    ...
    <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
      <h3 className="text-secondary text-[15px] font-semibold mb-1">Community Activity Trend</h3>
      <p className="text-slate-400 text-[11px] font-medium mb-6">Posts, replies, and reactions over 8 weeks</p>
    ...
    {/* Parent Engagement */}
    <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
      <h3 className="text-secondary text-[15px] font-semibold mb-1">Parent Engagement Metrics</h3>
      <p className="text-slate-400 text-[11px] font-medium mb-6">How actively parents are using the platform</p>
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {parentEngagementData.map((p, i) => (
          <div key={i} className="bg-slate-50/50 rounded-2xl p-5 border border-slate-100 group hover:border-primary/30 transition-all">
            <div className="flex items-center justify-between mb-3">
              <p className="text-[12px] font-medium text-slate-400">{p.metric}</p>
              <span className={cn(
            ...
            <p className="text-2xl font-bold text-secondary">{p.value.toLocaleString()}</p>
          </div>
        ))}
      </div>
    </div>
    ...
    {/* Moderation Stats */}
    <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
      <h3 className="text-secondary text-[15px] font-semibold mb-1">Content Moderation Overview</h3>
      <p className="text-slate-400 text-[11px] font-medium mb-6">Teacher moderation queue status</p>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Pending Review", value: "12", icon: "pending", color: "bg-amber-50 text-amber-600" },
          { label: "Approved Today", value: "34", icon: "check_circle", color: "bg-emerald-50 text-emerald-600" },
          { label: "Rejected", value: "3", icon: "block", color: "bg-rose-50 text-rose-500" },
          { label: "Avg Review Time", value: "2.4h", icon: "timer", color: "bg-blue-50 text-blue-600" },
        ].map((m, i) => (
          <div key={i} className="flex flex-col items-center gap-3 p-5 rounded-2xl border border-slate-100 bg-slate-50/30">
