  ...
  <div className="space-y-8">
      ...
      {[
        ...
        <div key={i} className="flex items-center gap-3 rounded-2xl px-5 py-4 bg-white border border-slate-100 shadow-sm">
          <div className="size-10 rounded-xl flex items-center justify-center bg-accent shrink-0">
            <span className="material-symbols-outlined text-[20px] text-secondary/70">{s.icon}</span>
          </div>
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-medium">{s.trend}</p>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
        <h3 className="text-secondary text-[15px] font-semibold mb-1">Quiz Score Distribution</h3>
        <p className="text-slate-400 text-[11px] font-medium mb-6">Average score per quiz</p>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
        <h3 className="text-secondary text-[15px] font-semibold mb-1">Quiz Participation by Class</h3>
        <p className="text-slate-400 text-[11px] font-medium mb-6">Engagement levels across classes</p>
        <div className="space-y-3">
          {quizParticipationByClass.map((c, i) => (
            <div key={i} className="flex items-center gap-4">
              <span className="text-[12px] font-bold text-secondary w-12 shrink-0">{c.class}</span>
              <div className="flex-1 h-6 bg-slate-50 rounded-lg overflow-hidden relative">
                <div
                  className={cn(
                    ...
                    c.avgParticipation >= 85 ? "bg-emerald-500" : c.avgParticipation >= 70 ? "bg-amber-400" : "bg-rose-400"
                  )}
              ...
              <span className="text-[11px] text-slate-400 font-medium w-16 text-right shrink-0">{c.quizzesTaken} quizzes</span>
            </div>
          ))}
        </div>
      </div>
    ...
    {/* Time Analysis */}
    <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
      <h3 className="text-secondary text-[15px] font-semibold mb-1">Time vs Score Analysis</h3>
      <p className="text-slate-400 text-[11px] font-medium mb-6">How quiz timing correlates with performance</p>
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
        {quizPerformanceData.map((q, i) => (
          <div key={i} className="bg-slate-50/50 rounded-2xl p-4 border border-slate-100 text-center group hover:border-primary/30 transition-all">
            <p className="text-[11px] font-bold text-slate-400 mb-2 truncate">{q.quiz}</p>
            <div className="relative size-16 mx-auto mb-3">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-[12px] font-black text-secondary">{q.avgTime}m</span>
                <span className="text-[8px] text-slate-400">/{q.allottedTime}m</span>
              </div>
            </div>
            <p className="text-[12px] font-bold text-secondary">Score: {q.avgScore}%</p>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-secondary text-[15px] font-semibold">Competition Results</h3>
            <p className="text-slate-400 text-[11px] font-medium mt-0.5">Recent inter-school & school events</p>
          </div>
        </div>
      </div>
      <div className="space-y-3">
        {competitionResults.map((c, i) => (
          <div key={i} className="flex items-center justify-between p-4 rounded-2xl border border-slate-50 bg-slate-50/50 hover:bg-slate-50 transition-all group">
            <div className="flex items-center gap-4">
              <div className={cn(
                "size-10 rounded-xl flex items-center justify-center font-bold text-[14px]",
                c.position === "1st" ? "bg-amber-100 text-amber-700" : c.position === "2nd" ? "bg-slate-200 text-slate-600" : "bg-orange-100 text-orange-700"
              )}>
                {c.position}
              </div>
              <div>
                <p className="text-[13px] font-semibold text-secondary">{c.name}</p>
                <p className="text-[10px] text-slate-400 font-medium">{c.date} • {c.type}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-[12px] font-bold text-secondary">{c.student}</p>
