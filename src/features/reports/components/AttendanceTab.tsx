    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          { label: "Present Today", value: totals.present.toString(), icon: "check_circle", color: "text-emerald-600" },
          { label: "Absent", value: totals.absent.toString(), icon: "cancel", color: "text-rose-500" },
          { label: "Half Day", value: totals.halfDay.toString(), icon: "timelapse", color: "text-amber-500" },
          { label: "Overall", value: `${overallPct}%`, icon: "monitoring", color: "text-secondary" },
        ].map((s, i) => (
          <div key={i} className="flex items-center gap-3 rounded-2xl px-5 py-4 bg-white border border-slate-100 shadow-sm">
            <div className="size-10 rounded-xl flex items-center justify-center bg-accent shrink-0">
              <span className={cn("material-symbols-outlined text-[20px]", s.color || "text-secondary/70")}>{s.icon}</span>
            </div>
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-secondary text-[15px] font-semibold">Class-wise Attendance</h3>
              <p className="text-slate-400 text-[11px] font-medium mt-0.5">Today's snapshot</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-secondary text-[15px] font-semibold">Weekly Attendance Trend</h3>
              <p className="text-slate-400 text-[11px] font-medium mt-0.5">12-week rolling average</p>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={320}>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-secondary text-[15px] font-semibold">Chronic Absenteeism Alert</h3>
              <p className="text-slate-400 text-[11px] font-medium mt-0.5">Students with attendance below 75%</p>
            </div>
          </div>
          <button className="flex items-center gap-2 px-3 py-1.5 bg-rose-50 rounded-xl border border-rose-100 text-rose-600 text-[12px] font-bold hover:bg-rose-100 transition-colors">
                ...
                <tr key={i} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  <td className="py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[13px] font-semibold text-secondary">{s.name}</p>
                        <p className="text-[10px] text-slate-400">{s.id}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-3.5 text-[13px] text-slate-500 font-medium">{s.class}</td>
                  <td className="py-3.5">
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div className={cn("h-full rounded-full", s.status === "critical" ? "bg-rose-500" : "bg-amber-500")} style={{ width: `${s.attendance}%` }} />
                      </div>
                      <span className="text-[12px] font-bold text-secondary">{s.attendance}%</span>
                    </div>
                  </td>
                  <td className="py-3.5 text-[13px] font-semibold text-secondary">{s.daysAbsent}</td>
                  <td className="py-3.5">
                    <span className={cn(
                      "text-[10px] font-bold px-2 py-0.5 rounded-full border uppercase",
                      s.status === "critical" ? "bg-rose-50 text-rose-600 border-rose-100" : "bg-amber-50 text-amber-600 border-amber-100"
                    )}>{s.status}</span>
                  </td>
                  <td className="py-3.5">
                    <button className="text-[11px] font-medium text-secondary hover:text-primary transition-colors underline underline-offset-2">View Profile</button>
                  </td>
                </tr>
              ))}
            </tbody>
