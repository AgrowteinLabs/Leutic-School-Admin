...
export const AddStaffPage = () => {
    ...
    // Step 3 State
    const [classTeacherFor, setClassTeacherFor] = useState("Not Assigned");
    ...
    const steps = [
        { id: 1, title: "Personal Identity", subtitle: "Basic information and contact details", icon: "badge", color: "text-primary", bg: "bg-primary/10" },
        { id: 2, title: "Employment Details", subtitle: "Role, department and qualifications", icon: "work", color: "text-emerald-600", bg: "bg-emerald-50" },
        { id: 3, title: "Access & Logistics", subtitle: "System roles and work schedule", icon: "key", color: "text-amber-600", bg: "bg-amber-50" },
    ];
            ...
            <TopBar
                ...
                actions={
                    <div className="flex items-center gap-3">
                        ...
                        <PDSButton variant="primary" icon="person_add" onClick={handleFinalize} disabled={activeStep < 3}>Complete Onboarding</PDSButton>
                    </div>
                }
            />
                                                ...
                                                <div className="p-10 pt-4 space-y-12">
                                                        ...
                                                        <div className="flex flex-col lg:flex-row gap-16">
                                                            <div className="shrink-0">
                                                                <div className="size-40 rounded-[40px] bg-[#F7F8F4] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-3 text-[#B0AFA8] hover:border-primary hover:text-primary transition-all cursor-pointer group">
                                                                    ...
                                                                    <span className="text-[11px] font-bold uppercase tracking-wider">Profile Photo</span>
                                                                </div>
                                                            </div>
                                                            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                                                                <PDSFormGroup label="Full Name" placeholder="Enter staff name" value={fullName} onChange={setFullName} />
                                                                ...
                                                                <PDSFormGroup label="Personal Email" placeholder="personal@example.com" icon="mail" value={personalEmail} onChange={setPersonalEmail} />
                                                                <PDSFormGroup label="Mobile Number" placeholder="+91 XXXX" icon="call" value={mobile} onChange={setMobile} />
                                                                <PDSFormGroup label="Residential Address" type="textarea" placeholder="Enter permanent address..." className="md:col-span-2" value={address} onChange={setAddress} rows={2} />
                                                            </div>
                                                        </div>
                                                    )}
                                                    ...
                                                    {step.id === 2 && (
                                                        <div className="space-y-16">
                                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-x-12 gap-y-10">
                                                                <PDSFormGroup label="Employee ID" placeholder="e.g. STAFF-2024-001" value={empId} onChange={setEmpId} />
                                                                <PDSFormGroup label="Joining Date" type="date" value={joiningDate} onChange={setJoiningDate} />
                                                                <PDSFormGroup 
                                                                    label="Subject Area" 
                                                                    type="select" 
                                                                    options={["Mathematics", "English", "Science", "Social Studies", "Hindi", "Physical Education", "Arts & Crafts", "Computer Science", "Administration"]} 
                                                                    searchable 
                                                                    value={dept} 
                                                                    onChange={setDept} 
                                                                />
                                                                <PDSFormGroup label="Designation" placeholder="e.g. Senior Faculty" value={designation} onChange={setDesignation} />
                                                                <PDSFormGroup label="Qualifications" placeholder="e.g. PhD, M.Ed" value={qualifications} onChange={setQualifications} />
                                                                <PDSFormGroup label="Years of Experience" placeholder="e.g. 8 Years" value={exp} onChange={setExp} />
                                                                <PDSFormGroup label="Institutional Email" placeholder="faculty@institution.com" icon="workspace_premium" value={instEmail} onChange={setInstEmail} />
                                                                <PDSFormGroup label="Emergency Contact" placeholder="+91 XXXX" icon="call" value={emergencyContact} onChange={setEmergencyContact} />
                                                            </div>

                                                            <div className="border-t border-slate-100 pt-10 space-y-10">
                                                                <div className="flex flex-col gap-1 border-b border-slate-50 pb-4">
                                                                    <h5 className="text-[15px] font-bold text-foreground">Teaching Profile</h5>
                                                                    <p className="text-[12px] text-[#B0AFA8] font-medium">Qualified grades and subjects</p>
                                                                </div>
                                                                <div className="grid grid-cols-1 gap-10">
                                                                    <div className="space-y-4">
                                                                        <div className="flex items-center justify-between px-1">
                                                                            <label className="text-[12px] font-bold text-[#B0AFA8] tracking-tight">Qualified Grades</label>
                                                                            <button 
                                                                                type="button"
                                                                                onClick={() => {
                                                                                    const allGrades = ["Grade 1", "Grade 2", "Grade 3", "Grade 4", "Grade 5", "Grade 6", "Grade 7", "Grade 8", "Grade 9", "Grade 10", "Grade 11", "Grade 12"];
                                                                                    if (qualifiedGrades.length === allGrades.length) setQualifiedGrades([]);
                                                                            ...
                                                                            >
                                                                                {qualifiedGrades.length === 12 ? "Deselect All" : "Select All"}
                                                                            </button>
                                                                        </div>
                                                                        <PDSFormGroup 
                                                                            label="" // Hidden label since we have a custom one
                                                                            type="chips" 
                                                                    ...
                                                                    <div className="space-y-4">
                                                                        <div className="flex items-center justify-between px-1">
                                                                            <label className="text-[12px] font-bold text-[#B0AFA8] tracking-tight">Subject Specializations</label>
                                                                            <button 
                                                                                type="button"
                                                                                onClick={() => {
                                                                                    const allSpecs = ["Mathematics", "Physics", "Chemistry", "Biology", "English", "Social Science", "Information Technology", "Physical Education", "Fine Arts"];
                                                                                    if (specializations.length === allSpecs.length) setSpecializations([]);
                                                                                    else setSpecializations(allSpecs);
                                                                                }}
                                                                                className="text-[11px] font-bold text-primary hover:text-secondary transition-colors underline underline-offset-4"
                                                                            >
                                                                                {specializations.length === 9 ? "Deselect All" : "Select All"}
                                                                            </button>
                                                                        </div>
                                                                        <PDSFormGroup 
                                                                            label="" 
                                                                            type="chips" 
                                                                            options={["Mathematics", "Physics", "Chemistry", "Biology", "English", "Social Science", "Information Technology", "Physical Education", "Fine Arts"]} 
                                                    ...
                                                    {step.id === 3 && (
                                                        <div className="space-y-12">
                                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-x-12 gap-y-10">
                                                                <PDSFormGroup label="Class Teacher For" type="select" options={["Not Assigned", "Grade 10-A", "Grade 11-B", "Grade 12-C"]} searchable value={classTeacherFor} onChange={setClassTeacherFor} />
                                                                <PDSFormGroup label="Portal Access Role" type="select" options={["Standard Teacher", "Dept. Head", "Admin"]} value={portalRole} onChange={setPortalRole} />
                                                                <PDSFormGroup label="Work Shift" type="select" options={["Morning (8:00 - 15:00)", "Noon (11:00 - 18:00)"]} value={shift} onChange={setShift} />
                                                                <PDSFormGroup label="Bus Availed" type="select" options={["No", "Yes - Route A", "Yes - Route B"]} value={busAvailed} onChange={setBusAvailed} />
                                                            </div>
                                                            ...
                                                            <div className="bg-slate-50 p-8 rounded-[32px] border border-slate-100 flex items-center justify-between">
                                                                <div className="flex items-center gap-6">
                                                                    ...
                                                                    <div>
                                                                        <p className="text-[15px] font-bold text-foreground">Biometric Enrollment</p>
                                                                        <p className="text-[13px] text-[#B0AFA8] font-medium">Link staff RFID or Fingerprint device for automated attendance</p>
                                                                    </div>
                                                                </div>
                                                                <PDSButton variant="text" icon="link" className="px-6 h-10 bg-white">Link Device</PDSButton>
                                                            </div>
                                                        </div>
                                                    )}
